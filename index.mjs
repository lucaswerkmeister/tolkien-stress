...
export default function analyse(word, language) {
    word = word.normalize('NFC'); // canonical composition: ensure ó, ë etc. are single chars
    ...
    for (let i = word.length - 1; i >= 0; i--) {
        // note: we iterate over chars, not code points; should make no difference since all needed characters are BMP
        ...
        let long = false;
        /** Is it specifically a diphthong? (This flag is only used temporarily; at the end, diphthongs are counted as long, but i is decremented to account for the second letter.) */
        let diphthong = false;
        switch (letter) {
            case 'i':
                vowel = true;
